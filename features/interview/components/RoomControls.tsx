    ...
    <footer className="room-controls" data-testid="room-controls">
      <div className="room-controls-left">
        <div className="room-controls-group">
          <button
            className="room-control-btn-dojo"
            onClick={onToggleMic}
            disabled={!sttSupported || phase === 'interviewerSpeaking' || phase === 'saving'}
            aria-pressed={micOn}
            aria-label={micOn ? '마이크 끄기' : '마이크 켜기'}
          >
            <span className={`room-control-dot${micOn ? ' is-on' : ' is-off'}`} aria-hidden="true" />
            {micOn ? '마이크 켜짐' : '마이크 꺼짐'}
          </button>
          ...
          <button
            className={`room-control-btn-dojo${panelOpen ? ' is-active' : ''}`}
        ...
        <div className="room-controls-group">
          <button
            className={`room-control-btn-dojo${videoRecording ? ' is-recording' : ''}`}
          ...
          <button
            className={`room-control-btn-dojo${audioRecording ? ' is-recording' : ''}`}
